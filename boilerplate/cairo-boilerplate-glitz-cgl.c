...
typedef struct _glitz_cgl_target_closure {
    glitz_target_closure_base_t base;
} glitz_cgl_target_closure_t;

static glitz_surface_t *
_cairo_boilerplate_glitz_cgl_create_surface_internal (glitz_format_name_t		 formatname,
						      int				 width,
						      int				 height,
						      glitz_cgl_target_closure_t	*closure)
{
    ...
    dformat = glitz_cgl_find_pbuffer_format (mask, &templ, 0);
    if (!dformat) {
	CAIRO_BOILERPLATE_LOG ("Glitz failed to find pbuffer format for template.");
	goto FAIL;
    }

    gdraw = glitz_cgl_create_pbuffer_drawable (dformat, width, height);
    if (!gdraw) {
					     ...
					     void			**closure)
{
    ...
    glitz_cgl_target_closure_t *cglc;
    int status;

    glitz_cgl_init ();

    *closure = cglc = xmalloc (sizeof (glitz_cgl_target_closure_t));

    switch (content) {
    case CAIRO_CONTENT_COLOR:
	glitz_surface = _cairo_boilerplate_glitz_cgl_create_surface_internal (GLITZ_STANDARD_RGB24, width, height, NULL);
	break;
    case CAIRO_CONTENT_COLOR_ALPHA:
	glitz_surface = _cairo_boilerplate_glitz_cgl_create_surface_internal (GLITZ_STANDARD_ARGB32, width, height, NULL);
	break;
    case CAIRO_CONTENT_ALPHA:
    default:
	CAIRO_BOILERPLATE_LOG ("Invalid content for glitz-cgl test: %d\n", content);
	goto FAIL;
    }
    ...
    cglc->base.width = width;
    cglc->base.height = height;
    cglc->base.content = content;
    status = cairo_surface_set_user_data (surface,
					  &glitz_closure_key, cglc, NULL);
